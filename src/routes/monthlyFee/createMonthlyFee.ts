...
const createMonthlyFeeSchema = z.object({
  ...
  effectiveFromPeriod: z.number().positive(),
});

export async function createMonthlyFee(req: Request, res: Response) {
  const { description, amount, effectiveFromPeriod } =
    createMonthlyFeeSchema.parse(req.body);

  const monthlyFeeFeature = container.get<MonthlyFeeFeature>(
    TYPES.MonthlyFeeFeature
  );

  const monthlyFee = await monthlyFeeFeature.create(
    description,
    amount,
    effectiveFromPeriod
  );
